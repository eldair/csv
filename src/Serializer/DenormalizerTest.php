...
final class DenormalizerTest extends TestCase
{
    ...
    public function testItWillThrowIfTheColumnAttributesCasterIsInvalid(): void
    {
        ...
        $this->expectException(MappingFailed::class);
        $this->expectExceptionMessage('`stdClass` must be an resolvable class implementing the `Eldair\Csv\Serializer\TypeCasting` interface or a supported alias.');
    ...
    public function testItWillThrowIfThePropertyIsMisMatchWithTheTypeCastingClass(): void
    {
        ...
        $this->expectException(MappingFailed::class);
        $this->expectExceptionMessage('The type for the method `'.$foobar::class.'::setFirstName` first argument `firstName` is invalid; `DateTimeInterface` or `mixed` type must be used with the `Eldair\Csv\Serializer\CastToDate`.');
