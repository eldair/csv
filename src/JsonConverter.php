...
final class JsonConverter
{
    ...
    private const REGEXP_INDENTATION = '/^/m';
    ...
    private function __construct(
        ...
        [$this->start, $this->end] = ($this->preserveOffset || $forceObject) ? ['{'.$eol, $eol.'}'] : ['['.$eol, $eol.']'];
        $this->separator = ','.$eol;
        $this->formatter = match (true) {
            $isPrettyPrint && $this->preserveOffset => fn (int $increment, string|int $key, string $json): string => (string) preg_replace(self::REGEXP_INDENTATION, self::INDENTATION, '"'.$key.'": '.$json),
            $isPrettyPrint && !$this->preserveOffset && $forceObject => fn (int $increment, string|int $key, string $json): string => (string) preg_replace(self::REGEXP_INDENTATION, self::INDENTATION, '"'.$increment.'": '.$json),
            $isPrettyPrint && !$this->preserveOffset && !$forceObject => fn (int $increment, string|int $key, string $json): string => (string) preg_replace(self::REGEXP_INDENTATION, self::INDENTATION, $json),
            !$isPrettyPrint && $this->preserveOffset => fn (int $increment, string|int $key, string $json): string => '"'.$key.'":'.$json,
            !$isPrettyPrint && !$this->preserveOffset && $forceObject => fn (int $increment, string|int $key, string $json): string => '"'.$increment.'":'.$json,
            default => fn (int $increment, string|int $key, string $json): string => $json,
        };
    }
